	...
	refresh(frm) {
		...
		frappe.call({
			...
			callback(r) {
				...
				if (r.message && r.message.gateway_verification_enabled) {
					add_verify_payment_receipts_button(frm);
				}
			},
		});
	},
...
function add_verify_payment_receipts_button(frm) {
	frm.add_custom_button(__("Verify Payment Receipts"), () => {
		frappe.call({
			...
			callback(r) {
				const results = r.message || [];
				const verified = results.filter((row) => row.verification_status === "Verified");
